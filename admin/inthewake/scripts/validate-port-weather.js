...
if (allIdx !== -1 && args[allIdx + 1]) {
  ...
  // Filter out option flags; only exclude the --all directory arg when --all is present
  const allDirArg = allIdx !== -1 ? args[allIdx + 1] : null;
  files = args.filter(a => !a.startsWith('--') && a !== allDirArg);
}
