#!/usr/bin/env python3
"""
Replace placeholder images for ports that have duplicate/identical images.
Sources real images from Wikimedia Commons.
"""
...
# Ports with placeholder images and their search terms
PLACEHOLDER_PORTS = {
    "antigua": ["Antigua Caribbean", "English Harbour Antigua", "St Johns Antigua"],
    "apia": ["Apia Samoa", "Samoa beach", "Apia waterfront"],
    "aqaba": ["Aqaba Jordan", "Aqaba Red Sea", "Petra Jordan"],
    "aruba": ["Aruba beach", "Oranjestad Aruba", "Aruba Caribbean"],
    "ascension": ["Ascension Island", "Georgetown Ascension", "Green Mountain Ascension"],
    "athens": ["Athens Piraeus", "Acropolis Athens", "Plaka Athens"],
    "auckland": ["Auckland New Zealand", "Auckland Harbour", "Sky Tower Auckland"],
    "bali": ["Bali Indonesia", "Ubud Bali", "Tanah Lot Bali"],
    "barcelona": ["Barcelona harbor", "La Rambla Barcelona", "Sagrada Familia"],
    "civitavecchia": ["Civitavecchia Italy", "Rome Colosseum", "Vatican City"],
    "dubrovnik": ["Dubrovnik Croatia", "Dubrovnik Old Town", "Dubrovnik walls"],
    "lanzarote": ["Lanzarote Canary", "Timanfaya National Park", "Cesar Manrique"],
    "manzanillo": ["Manzanillo Mexico", "Manzanillo beach", "Manzanillo harbor"],
    "mazatlan": ["Mazatlan Mexico", "Mazatlan beach", "Old Mazatlan"],
    "mykonos": ["Mykonos Greece", "Mykonos windmills", "Little Venice Mykonos"],
    "naples": ["Naples Italy", "Pompeii ruins", "Mount Vesuvius"],
    "progreso": ["Progreso Mexico", "Progreso Yucatan", "Merida Mexico"],
    "puerto-vallarta": ["Puerto Vallarta Mexico", "Puerto Vallarta beach", "Malecon Puerto Vallarta"],
    "santorini": ["Santorini Greece", "Oia Santorini", "Santorini caldera"],
    "tampa": ["Tampa Florida", "Tampa Bay", "Ybor City Tampa"],
    "venice": ["Venice Italy", "St Marks Square Venice", "Grand Canal Venice"],
    "zihuatanejo": ["Zihuatanejo Mexico", "Zihuatanejo beach", "Ixtapa Mexico"],
}
...
def replace_port_images(port_name, searches):
    ...
    # Backup and clear existing images
    ...
    for idx, img_type in enumerate(STANDARD_IMAGES):
        if count >= 12:  # Limit to 12 images per port
            break
        ...
        results = search_wikimedia(f"{query} -svg -diagram -map -logo", limit=8)
        time.sleep(0.3)
        ...
        for filename in results:
            ...
            if any(x in lower_name for x in ['icon', 'logo', 'flag', 'coat', 'seal', 'symbol']):
                continue
...
def main():
    ports = list(PLACEHOLDER_PORTS.keys())

    # Allow specifying specific ports on command line
    if len(sys.argv) > 1:
        ports = [p for p in sys.argv[1:] if p in PLACEHOLDER_PORTS]

    total = 0
    for port in ports:
        print(f"\n{port}:")
        searches = PLACEHOLDER_PORTS[port]
        count = replace_port_images(port, searches)
        total += count
        print(f"  {count} images downloaded")
        time.sleep(1)  # Be nice to Wikimedia servers
