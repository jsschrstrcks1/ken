...
| 8 | `context-budget` skill | ECC `skills/context-budget/` | `open-claw-stuff/skills/context-budget/` | **✓ shipped** v1.0.0 in open-claw-stuff v0.7.0; concept-lift adds detection-only posture (no auto-prune), skill-catalog-sprawl as peer-class lever alongside MCP, top-10 *questions* (not suggestions), approximation disclaimer (bytes÷4 / bytes÷3.5, ±20%); 5-pattern backdoor defenses; smoke-test against ken: ~7.5K always-loaded approx tokens (CLAUDE.md + skill catalog frontmatter + skill-rules.json + hooks + settings), ~48.6K body load, sprawl PASS (30 skills, 31 ever-created — net growth minimal), MCP gap correctly labeled rather than silent-skipped |
| 9 | Continuous-learning-v2 / instincts (confidence-scored, project-scoped, auto-promoting memory) | ECC `skills/continuous-learning-v2/` | `ken/orchestrator/memory_ops.py` v3.1 instincts tier | **partial-shipped (Slice 1)** — feature-flagged additive API: `extract_instinct_candidates()` + `promote_to_instinct()` + `demote_from_instinct()`; `MEMORY_LEARNING_ENABLED` env defaults off; 24 new tests + 46 pre-existing all pass; no hooks, no background daemon, no auto-promotion (each its own future slice); concept-only lift with 9 intentional departures from ECC documented in `open-claw-stuff/schemas/records/continuous-learning-v2.json` |
| 10 | `ai-regression-testing` | ECC `skills/ai-regression-testing/` | `open-claw-stuff/skills/ai-regression-testing/` | **✓ shipped** v1.0.0 in open-claw-stuff v0.8.0; significant scope inversion (ECC tests AI-written app code; ours tests shape stability of AI-touched artifacts — skill frontmatters, provenance records, schemas); central guard is Stage 6 CRITICAL when same commit moves both an artifact AND its baseline (the "agent marks its own homework" failure mode); detection-only, no --rebaseline flag exists; initial baseline shipped covering 10 skills + 5 provenance records; Stage 5 path-parity audit at capture confirms perfect frontmatter parity across all 10 skills |
...
- ✓ P1#8 `context-budget` shipped (open-claw-stuff v0.7.0); concept-lift adds detection-only posture, skill-catalog-sprawl as peer-class lever alongside MCP, top-10 *questions* not suggestions, ±20% approximation disclaimer, 5-pattern backdoor defenses; smoke-tested against ken (~7.5K always-loaded approx tokens, sprawl PASS, MCP gap correctly labeled)
- ✓ P1#10 `ai-regression-testing` shipped (open-claw-stuff v0.8.0); scope inversion from ECC's app-code regression to artifact-shape regression for AI-touched files; Stage 6 CRITICAL when same commit moves both an artifact and its baseline; initial baseline ships covering 10 skills + 5 provenance records; Stage 5 path-parity audit confirms all 10 SKILL.md frontmatters share the same 8 fields
- ✓ Test suite for `orchestrator/memory_ops.py` (P1#9 prerequisite) — 46 tests anchoring v3 behavior across encode/recall/update/link/protect/archive/promote/forget/extract/consolidate/neighbors + internal helpers; tempdir-isolated; pre/post diff confirmed `~/.memory/` untouched
- ✓ P1#9 Slice 1 shipped — v3.1 instincts tier in `memory_ops.py`: 3 additive functions feature-flagged behind `MEMORY_LEARNING_ENABLED` env (default off); 0 modifications to existing functions; 24 new tests pass alongside the 46 pre-existing (70 total); concept-only lift from ECC continuous-learning-v2 with 9 intentional departures (no hooks, no background daemon, no auto-promotion, no parallel namespace, ships dark); future slices not scheduled — each its own audit-reviewable decision
- ✓ P1#9 Slices 1.1 + 1.2 shipped — edge-test hardening of memory_ops auto-operations: H1 instinct shield from auto-archive, K1 corrupt-JSON robustness; AM1/AM4 shield-aware auto-merge, AM3 deterministic tie-break, U1 refuse update on superseded, AP1+AP2 real-edges-only auto-protect; 82 total tests
- ✓ P1#9 multi-LLM plan review (3 rounds, $0.38, 12 successful + 2 truncated Gemini) — threats 7→23, invariants 4→12, slices 7→12 across plan v1→v5; final `CONTINUOUS_LEARNING_PLAN.md` (935 lines) + `CONTINUOUS_LEARNING_REVIEWS.md` ledger; v5 introduces `MEMORY_LEARNING_PROFILE` (default `single-operator-local`) so multi-actor-threat defenses become non-applicable in this household
- ✓ P1#9 Slice 0 shipped — doctrine layer: `CarefulNotCleverError` exception, `_learning_profile()` reader, 9 invariants (`_assert_panic_check` + `_assert_no_silent_skip` + `_assert_evidence_present` + `_assert_single_id` + `_assert_safety_guard_compliant` + `_assert_evidence_integrity` stub + `_assert_rate_limit` dynamic threshold + `_assert_temporal_consistency` 5min/5min + `_assert_human_attention` profile-aware); `CONTINUOUS_LEARNING_DOCTRINE.md` (~150 lines plain-language principles); CI gate `test_every_mutation_path_invokes_invariants` (AST walk + legacy allowlist for v1-1.2 functions); 40 new tests pass alongside 82 pre-existing (122 total); ~/.memory byte-diff empty
- ✓ P1#9 Slice 0.5 shipped — mutation defense scaffolding: 5 new helpers in `memory_ops.py` covering M1-M9 vectors (`_validate_helper_integrity` SHA256-bytecode-and-identity check for M2 monkey-patching, `_seal_helpers` lifecycle for M6 symbol-shadowing, `_validate_file_integrity` HMAC sidecar for M3 direct-write, `_validate_key_fingerprint` SHA256-of-key for M9 HMAC swap (Grok R3's highest-impact gap), `_audit_config_integrity` for M5 settings.json drift); `tests/test_meta_ci.py` (24 tests across 7 classes: monkey-patch detection, direct-write detection, config-drift detection, key-replacement detection, regression-test-count baseline, CI gate extensions for swallow patterns + conditional invariants + skip-decorators-on-safety-tests, seal-helpers lifecycle); 146 total tests pass; live monkey-patch detection verified independent of test suite; ~/.memory byte-diff empty
- ✓ P1#9 Slice 1 shipped — kill-switch completion: wired `_assert_panic_check()` as first executable statement of all 3 Slice 1.1 learning functions (`extract_instinct_candidates`, `promote_to_instinct`, `demote_from_instinct`); new AST rule `test_panic_check_is_first_executable_statement` enforces order on every learning function that calls panic-check; adversarial probe `PanicFlipMidOperationTests` (3 tests) verifies panic flipped mid-session halts subsequent calls AND that Slice 1.1 functions inherit panic halting; legacy allowlist tightened (promote_to_instinct + demote_from_instinct removed since they now call invariants); 150 total tests pass; ~/.memory byte-diff empty
- ✓ P1#9 Slice 2 shipped — pull-based session extraction: `extract_candidates_from_session(session_id, dry_run=True, state_dir=None)` reads `orchestrator/state/<session_id>.json` and surfaces three candidate kinds (`fact` from verified_claims with confidence 0.95, `pattern` from repeated pipeline roles ≥3 with scaled confidence, `preference` from task description with confidence 0.7); each candidate carries full provenance (`evidence.session_id`, `evidence.observations[]`, `evidence.observation_count`) plus a `delta` field describing what behavior would change if promoted (Differential Memory Review per Perplexity R2); 4 invariant calls in Slice-1-enforced order (panic_check, rate_limit, evidence_present, evidence_integrity-stub); single-read snapshot pattern for TOCTOU mitigation; `_validate_session_id` defends against path traversal; `MEMORY_LEARNING_FROM_SESSIONS` defaults ON in single-operator-local profile and OFF in multi-operator-shared; 27 new tests (4 flag, 9 candidate, 14 adversarial including familiarity-crafted Grok-R2-regression + plan-injection wrapping + snapshot-isolation + oversized state + 6 path-traversal/typing variants); 153 total tests pass; ~/.memory byte-diff empty
- ✓ P1#9 Slice 4 shipped — confidence promotion via `consolidate()`: new `_confidence_promotion_enabled()` profile-aware flag (default ON in single-operator-local, OFF in multi-operator-shared); `_should_bump_confidence()` predicate (recall_count >= 5, last_recalled within 14 days, confidence < 1.0, defensive against missing/malformed timestamp + future-dated last_recalled); bump step inserted between decay and auto-archive in consolidate's per-domain loop (+0.05 cap 1.0); new `actions["confidence_promoted"]` counter; 23 new tests (5 flag, 8 criteria including boundaries at recall_count=5 and 14-day cutoff and cap-at-1.0, 10 adversarial: superseded skipped + protected still bumped + instinct still bumped + consecutive consolidates accumulate + future-dated last_recalled rejected + malformed timestamp rejected + archived flag respected + decay-then-bump same pass + once-per-pass even at recall_count=1000 + zero-conf eligible memory bumps); 176 total tests in test_memory_ops + 24 in test_meta_ci = 200 total; existing tests unchanged; ~/.memory byte-diff empty
- ✓ P1#9 Slice 5 shipped — usage history via `recall()`: `usage_history: [{at, session_id}, ...]` field appended on every recall bump; cap at most-recent 20 entries (FIFO); privacy invariant: only timestamp + `session_id` retained, no query content; `_usage_history_enabled()` profile-aware flag (default ON in single-operator-local, OFF in multi-operator-shared); `_current_session_id()` reads `MEMORY_SESSION_ID` env, defaults `"unknown"`; per-entry `_assert_temporal_consistency` validation against existing chain; defensive reinit if field corrupted to non-list; backdating attack surfaces as `_assert_no_silent_skip` raise; 16 new tests (5 flag including session-id-from-env, 5 append including legacy-memory-init + privacy-only-2-keys + distinct-sessions-accumulate, 6 adversarial: cap-at-20 + corrupted-field + session-collision + rapid-fire + clock-jumped-backward + FIFO-cap-overflow); 192 tests in test_memory_ops + 24 in test_meta_ci = 216 total; existing tests unchanged; CI gate + panic ordering rule still pass; ~/.memory byte-diff empty
- ✓ P1#9 Slice 7.5 shipped — consensus auto-promotion eligibility: new `_assert_promotion_eligibility(candidate)` invariant (10th in seal list) validates 5 cryptographic consensus criteria (recall_count≥10, age≥30d, confidence≥0.9, ≥5 distinct sessions in usage_history, no demoted_at, not archived/superseded) plus chains `_assert_evidence_integrity` (Slice 3C stub) + `_validate_helper_integrity` (Slice 0.5 mutation defense); new `auto_promote_eligible(domain=None)` public API iterates extract_instinct_candidates output, calls `_assert_promotion_eligibility` per candidate, promotes eligible via per-id `promote_to_instinct` then sets `is_auto_promoted: true` + `auto_promoted_at: <iso>` audit flags; `_auto_promote_enabled()` profile-aware flag (ON in single-operator-local default, **structurally FORBIDDEN in multi-operator-shared regardless of env var**); single-id contract preserved (iteration over per-id promotion, no batch API); 35 new tests (11 criteria boundary, 6 profile/flag, 6 end-to-end including audit-flag-distinguish + keeps-original-domain, 12 adversarial: 4 boundary-exact tests at 10/30d/0.9/5 + forged-recall-without-history + monkey-patched-helper-halts + rate-limit-exceeded + corrupted-usage_history + already-demoted-not-re-promoted + audit-flags-distinguish-auto-from-manual + single-id-contract + evidence_integrity-stub-call-site); 227 tests in test_memory_ops + 24 in test_meta_ci = 251 total; existing 216 tests unchanged; CI gate + panic ordering rule still pass; ~/.memory byte-diff empty
