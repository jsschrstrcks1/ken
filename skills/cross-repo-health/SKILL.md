...
| Repo | Path | Domain |
|------|------|--------|
| Romans | ~/repos/Romans (gh repo clone jsschrstrcks1/Romans) | Sermons |
| ken | ~/repos/ken (gh repo clone jsschrstrcks1/ken) | Hub |
| Allrecipes | ~/repos/Allrecipes | Recipe aggregator |
| flickersofmajesty | ~/repos/flickersofmajesty | Photography |
| Grandmasrecipes | ~/repos/Grandmasrecipes | Recipes |
| Grannysrecipes | ~/repos/Grannysrecipes | Recipes |
| InTheWake | ~/repos/InTheWake | Cruise planning |
| manateecreeksheep | ~/repos/manateecreeksheep | Sheep flock |
| MomsRecipes | ~/repos/MomsRecipes | Recipes |

## Health Checks Per Repo

1. **Git status** — uncommitted changes, unpushed commits
2. **Memory health** — `python3 /Volumes/1TB External/openclaw/workspace-main/tools/orchestrator/memory_ops.py tree --domain <domain>`
...
for repo in Romans ken Allrecipes flickersofmajesty Grandmasrecipes Grannysrecipes InTheWake manateecreeksheep MomsRecipes; do
  echo "=== $repo ===" && cd ~/repos/$repo && git status -s
done

# Memory health across all domains
python3 /Volumes/1TB External/openclaw/workspace-main/tools/orchestrator/memory_ops.py stats
```
