# Continuous-Learning-v2 Auto-Extraction Plan — v3

**Status:** Round-2 multi-LLM red-team complete; this is v3 after incorporating GPT/Grok/Gemini/Perplexity adversarial findings on top of v2.
...
def _assert_rate_limit(operation: str, key: str, max_per_minute: int = 60) -> None:
    """NEW v2: bound call rate per operation per key (session_id, memory_id,
    etc.). Mitigates DoS via repeated extraction and observation flooding."""
    now = time.time()
    bucket = _rate_buckets.setdefault((operation, key), [])
    bucket[:] = [t for t in bucket if now - t < 60]
    if len(bucket) >= max_per_minute:
        raise CarefulNotCleverError(
            f"rate limit: {operation} for {key} exceeded {max_per_minute}/min"
        )
    bucket.append(now)

def _assert_temporal_consistency(ts: str, chain: list = None) -> None:
    """NEW v2: reject timestamps more than 60s in the future or out-of-order
    against the prior entries in chain. Mitigates clock-skew and backdating
    attacks against usage_history (T12)."""
    now = time.time()
    try:
    ...
    if chain:
        ...
        if ts_epoch < last - 3600:
            raise CarefulNotCleverError(
                f"temporal consistency: {ts} backdated >1h from chain head"
            )
```
...
| 2 | **1** Kill-switch | Every subsequent slice's first invariant call. Must exist before extraction. |
