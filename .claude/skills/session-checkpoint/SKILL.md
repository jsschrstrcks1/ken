...
**Modern (keeper):**
```bash
PYTHONPATH=/home/user/ken python3 -m keeper status --family default
```
...
**Modern (keeper):**
```bash
PYTHONPATH=/home/user/ken python3 -m keeper join --family <name> \
  --goal "<short summary>"
...
**Modern (keeper):**
```bash
PYTHONPATH=/home/user/ken python3 -m keeper beat \
...
3. **Mark the session complete** so the next session doesn't flag it as abandoned:
   ...
   **Modern (keeper, with quality gate):**
   ```bash
   PYTHONPATH=/home/user/ken python3 -m keeper complete --family <name> \
